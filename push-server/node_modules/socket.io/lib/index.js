...
function Server(srv, opts) {
    ...
    this.sockets = this.of('/');  //默认就会新建nsme='/'的namespace实例。io上直接调用namespace的方法就会调用默认的'/'实例方法
 ...
 * @api public
 */
//attach时调用。监听engine.io的成功建立连接事件connection,建立client来抽象这个连接。
...
Server.prototype.onconnection = function (conn) {
    ...
    var client = new Client(this, conn); //engine.io的'connection'事件处理函数,参数conn是在engine层标识一路连接的socket(engine.io/lib/socket.js)
    client.connect('/');  //client默认给这路连接建立一个nsp='/'的socket,因为如果nsp是默认的'/',客户端不会发送{type:parser.CONNECT, nsp:'/'}消息
    return this;
};
